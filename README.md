...
So long, in fact that the token generated by the operating system when you enter your password will likely expire, and as a result, some of the subsequent steps would fail.  To work around this, open a separate terminal window.  In that window, type:
```
watch sudo ls
```
This may fail if "watch" is not yet installed on your machine.  If this is the case, first download it by running:
```
curl -O http://ktwit.net/code/watch-0.2-macosx/watch
```
This should continually renew your sudo token, allowing boxen to continue through its multiple steps.
